...
func (b *MockBackend) Register(_ context.Context, req channel.AdjudicatorReq, subChannels []channel.SignedState) error {
	...
	err := b.checkStates(states, checkRegister)
	if err != nil {
		return err
	}
...
func (b *MockBackend) Withdraw(_ context.Context, req channel.AdjudicatorReq, subStates channel.StateMap, acc wallet.Address) error {
	...
	err := b.checkStates(states, checkWithdraw)
	if err != nil {
		return err
	}
...
//nolint:funcorder // Keep helper close to subscription/balance internals used by tests.
...
// NotifyCoordinatedWithVersion publishes a CoordinatedEvent for the channel with an explicit version.
func (b *MockBackend) NotifyCoordinatedWithVersion(ch channel.ID, version uint64) {
	b.mu.Lock()
	defer b.mu.Unlock()
