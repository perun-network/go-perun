...
func (p VirtualChannelProposal) Encode(w io.Writer) error {
	return perunio.Encode(w, p.Proposer, p.Peers, p.Parents, p.IndexMaps)
}

// Decode decodes a proposal from an io.Reader.
func (p *VirtualChannelProposal) Decode(r io.Reader) error {
	return perunio.Decode(r, &p.BaseChannelProposal, &p.Proposer, &p.Peers, &p.Parents, &p.IndexMaps)
}
...
func (acc *VirtualChannelProposalAcc) Decode(r io.Reader) error {
	return perunio.Decode(r, &acc.BaseChannelProposalAcc, &acc.Responder)
}
